...
@BindingAdapter("sleepStartStop")
fun TextView.setSleepStartStop(item: SleepNight?) {
    item?.let {
        var fromTo = "${convertLongToDateString(item.startTimeMilli)} " +
                "to ${convertLongToDateString(item.endTimeMilli)}"
        text = fromTo
    }
}
